...
| 7 | Project Themes | Proposed | [ADR-0001](adr/0001-project-themes.md) | **Rescoped 2026-09-12:** phase 1 is an optional `theme.json` with only a `hero` section (colors, density, speed, size, vignette, clear title, or disable the hero). Design tokens, light/dark base themes and a switcher are later phases. The print-aesthetic objection (AGENTS.md treats it as load-bearing; no dark mode to build on) applies to those later phases, not to phase 1, which only touches the landing hero |
