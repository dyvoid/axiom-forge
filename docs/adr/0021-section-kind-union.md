...
1. Add `classifySection(sectionDef)` to `shared/schema.ts`, returning a discriminated union:
   `{ kind: 'prose' } | { kind: 'links' } | { kind: 'fields'; fields }`.
2. Export `isFieldValueEmpty` from `shared` and delete the two inline copies.
3. Dispatch on `classifySection` at all four sites, each with a `never` exhaustiveness check in
